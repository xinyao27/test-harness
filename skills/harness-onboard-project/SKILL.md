...
description: Introduce this Test Harness to an existing codebase for the first time — propose a module structure, then promises, then evidence. Use when adopting the harness in a fresh project, scanning an unfamiliar repo for reviewable behavior, producing an initial module + promise map, or when the project has no `.promise.yaml` files yet. Do not use for routine feature work (use `harness-add-feature`) or for diagnosing command failures (use `harness-troubleshoot`).
---

# Onboarding the Harness to an Existing Project

This skill is for the **first time** a Harness lands in a project. The output is an initial layer of `modules/*.module.yaml` files, `promises/**/*.promise.yaml` files, and `scenarioTest` bindings — enough that a human can review the project top-down without reading every line of code.
...
Architecture        ← project-wide overview (not yet implemented)
  Module            ← one reviewable area a human could own — modules/*.module.yaml
    Promise         ← one behavior commitment — promises/**/*.promise.yaml
...
2. If any `.promise.yaml` files already exist, collect their ids and `feature:` strings.
...
2. Write `promises/<area>/<slug>.promise.yaml` per approved promise, with review metadata filled in.
3. Bind tests with `scenarioTest(promiseId, ...)`. If an existing test already proves a promise, wrap it. If not, write one. Assert on what the promise's `observes` list names — DB state, UI text, files, events, exit codes. Prefer exact matchers over loose ones; `expect(mock).toHaveBeenCalled()` alone is weak evidence outside `boundary: adapter`.
