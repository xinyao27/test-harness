...
- Studio 专属 CSS class，例如 `.studio-node-card`、`.studio-context-panel` 和 `.studio-canvas-frame`，必须通过从 `--radius-*` 派生的 `--studio-*radius` 变量设置圆角。
- Hover、focus、selected 和 active 填充必须保留交互表面的同一圆角层级。菜单行和自定义按钮不应该在圆角面板里出现直角 hover 矩形。
