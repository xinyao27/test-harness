...
| `accepted_promise_without_evidence` warning                     | A promise is `accepted` (or `changed_requires_review`) but no test result references its id yet. The reviewer signed off; the test still has to catch up. | Bind a `scenarioTest` (or the language adapter's equivalent) to the promise id and run `harness test`. The check stays a warning until evidence appears. |
| `implemented_promise_without_evidence` error                    | A promise is `lifecycle: implemented` (claiming code + tests both exist) but has zero matching results — a contradiction.                                 | Either add the missing test binding, or move the promise back to `accepted` until the binding lands. Do not silently drop `implemented`.                 |
