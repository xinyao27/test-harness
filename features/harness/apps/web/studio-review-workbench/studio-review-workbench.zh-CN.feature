...
Feature: Studio review workbench
  ...
  Rule: Studio 中可以看到 Rule review actions
    ...
    Example: Rule panel 提供 state review actions
      Given Studio 已经加载了包含 proposed Rules 的 Harness snapshot
      When reviewer 打开一个 Rule panel
      Then panel 会提供 accept、request changes、reject、deprecate 和 supersede actions
  ...
  Rule: Studio 中可以看到 Rule review history
    ...
    Example: Rule panel 展示它的 review-log history
      ...
      Then reviewer 可以看到之前的 actions、authors、dates、notes 和 summaries
