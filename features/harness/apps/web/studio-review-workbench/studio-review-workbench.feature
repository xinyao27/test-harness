...
Feature: Studio review workbench
  ...
  Rule: Rule review actions are available in Studio
    ...
    Example: A Rule panel offers lifecycle review actions
      Given Studio has loaded a Harness snapshot with pending Rules
      When a reviewer opens a Rule panel
      Then the panel offers approve, request changes, reject, deprecate, and supersede actions
  ...
  Rule: Rule review history is visible in Studio
    ...
    Example: A Rule panel shows its review-log history
      ...
      Then the reviewer can see previous actions, authors, dates, acknowledgements, and summaries
