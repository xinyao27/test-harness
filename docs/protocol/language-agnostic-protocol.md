...
Portable conformance samples live under `protocol/fixtures/`. A reference implementation should accept every valid fixture and reject every invalid fixture in the same way as the protocol schemas.
...
1. Load the same `.promise.yaml` files.
2. Reject the same invalid promise and result fixtures.
3. Read or write the same `.harness/results.yaml` shape.
4. Render equivalent promise status.
5. Preserve CLI behavior and exit codes.
