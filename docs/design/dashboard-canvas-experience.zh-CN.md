...
> 状态：讨论稿
> 目标：把 Harness Web 体验简化成 canvas-first 的 workbench，为 promise-driven vibe coding 做准备。

## 1. 方向

Web 体验不应该像一个通用 admin dashboard。它应该更像一个 **TestHarness workbench**：人和 agent 可以从已 review 的行为承诺开始，一层层长出测试和实现。
...
未来方向是一套 vibe coding 环境：Dashboard 背后有 daemon，可以连接 Codex、cloud workers 或其他 agent runtime。这个 daemon 以后可以 draft promises、绑定测试、运行 Harness、修改业务代码。但现在 Dashboard 只需要先准备好交互模型，不要现在就把 agent 控制能力做进去。

## 2. 北极星：不再先打开编辑器

长期目标是：日常产品开发不再需要先打开代码编辑器。用户应该可以只打开 Harness workbench，管理 behavior model，让 agent 执行受控变更，并通过 accepted promises 和 passing evidence 判断项目状态。
...
这是一条项目级建模规则，不只是 Dashboard 的展示规则。生成 module 的 skills、描述 module 的 schemas、拒绝模糊 module 的 validators，以及未来 daemon 的动作，都应该先把 module 当成 architecture boundary。

这不是说“任何绿色测试套件都证明产品一定正确”。它的意思是：系统应该把上线契约显式化。Accepted promises 足够完整，每个 accepted promise 都有有意义的证据，并且最新 Harness run passing。当这些条件成立时，Dashboard 就可以成为人类判断 feature 是否可以上线的地方。

## 3. 当前方向的问题

现在 Dashboard 一开始暴露的概念太多：overview、modules、promises、graph、review queues、generation、runs、status views 都在抢顶层导航。这会让产品显得比用户的第一个任务复杂很多。
...
3. **Promise level**：选中一个 Promise 后，在右侧 contextual panel 展示 promise 内容。
...
完整的 Module nodes 必须能被当作项目架构图来阅读。如果一个 Module 不能对应一个有意义的架构部分，它就不应该是 Module。如果一个重要架构部分没有出现在 Module 层，Dashboard 就隐藏了项目的真实形状。
...
选中 Promise 后，右侧打开一个依附于 canvas 的 contextual panel。
...
- 用 canvas 替换当前 landing / dashboard / sidebar 体验
- 默认先渲染 Module nodes
- 选中 Module 后展开 Promise nodes
- 选中 Promise 后打开 Promise detail panel
- 所有数据保持 read-only

### Stage 2: Evidence Drilldown

- 在 Promise panel 中展示绑定测试证据
...
1. 右侧 Promise panel 在选中后是否一直展示，还是允许折叠？
2. Evidence 和 implementation files 在 Stage 2 是否要变成 graph nodes，还是先留在 panel 里？
3. URL model 用 query-state（`/?promise=...`）还是 semantic routes（`/promises/...`），但两者都渲染同一个 canvas？
4. 如果不叫 "Dashboard"，这个产品界面应该叫什么：Workbench、Canvas、Project Map，还是 Harness Studio？
5. UI 应该展示哪种上线规则：所有 accepted promises passing、所有 P0/P1 accepted promises passing，还是一个可配置 release profile？
