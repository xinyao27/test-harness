...
Harness 自己定义的 YAML 文件也支持多语言，但不需要每种语言复制一个文件。自然语言字段使用 `LocalizedText`：可以是一个普通字符串，代表默认英文；也可以是 `{ en, zh-CN }` 这样的语言 map。`id`、`tag`、`package`、`module`、`lifecycle`、`review.state` 和 result identifiers 这类稳定机器字段永不本地化。
...
- Harness YAML 的文本字段使用 `LocalizedText` map，但 YAML ids、tags、lifecycle、review state 和 evidence identifiers 保持语言中立。
...
当一个新需求进入时，Agent 应该先更新行为 artifacts，并等待人类 approval，然后再写实现：
...
用户需求
  ...
  -> 人类 review 并 approve 被触及的 .feature 行为
...
- 在一个 `.feature` 中所有被触及的 Rule 都变成 `accepted` 且 `approved`，或在当前 review surface 明确获得 approval 之前，Agent 不能为它写 step definitions、可执行测试或实现逻辑。
