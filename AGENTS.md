...
The first implementation is **Vitest-first**. Vitest should provide the execution foundation: test discovery, test running, assertions, mocks, projects, reporters, browser mode, coverage, and Node API orchestration. This project should build the promise and review layer around Vitest instead of reimplementing a test runner.
...
- **Seed Harness**: the smallest self-hosting loop for storing this project's promises, attaching Vitest scenario bindings, collecting results by promise id, and reporting readable promise status.
...
- Do not assume a passing Vitest test still proves a promise. Track N:M promise-to-test evidence mappings, capture assertion fingerprints, and preserve evidence deltas when tests are generated or edited.
- For the seed Harness, use PR-based review metadata and allow checker warnings until the checker itself is accepted.
- Build on Vitest before inventing Harness-specific runner behavior.
- Prefer `vitest run` for agent and CI execution so test commands exit deterministically.
- Use Vitest reporters, projects, annotations, browser mode, and Node API when they fit the problem.
